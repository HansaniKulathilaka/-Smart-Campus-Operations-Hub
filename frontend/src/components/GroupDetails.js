...
const GroupDetails = () => {
    ...
    const handleJoinGroup = async () => {
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post(`http://localhost:8080/session/${id}/join`, {}, {
                headers: { 'x-auth-token': token }
            });
    ...
    const handleApproveMember = async (userId) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:8080/session/${id}/approve/${userId}`, {}, {
                headers: { 'x-auth-token': token }
            });
            setGroup(prev => ({
                ...prev,
    ...
    const handleRemoveMember = async (userId) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:8080/session/${id}/members/${userId}`, {
                headers: { 'x-auth-token': token }
            });
            setGroup(prev => ({
                ...prev,
    ...
    const handleAssignRole = async (userId, role) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:8080/session/${id}/role/${userId}`, { role }, {
                headers: { 'x-auth-token': token }
            });
            setGroup(prev => ({
                ...prev,
    ...
    const handleAddResource = async (e, overrideResource = null) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post(`http://localhost:8080/session/${id}/resources`, resourceToSave, {
                headers: { 'x-auth-token': token }
            });
            setGroup(prev => ({
                ...prev,
    ...
    const handleDeleteResource = async (resourceId) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.delete(`http://localhost:8080/session/${id}/resources/${resourceId}`, {
                headers: { 'x-auth-token': token }
            });
            setGroup(prev => ({
                ...prev,
    ...
    return (
        <div className="group-details-container">
            ...
            <div className="group-cover-area">
                <img
                    src={group.coverImage || "https://img.freepik.com/free-vector/flat-back-school-pattern-collection_23-2149040892.jpg?semt=ais_hybrid"}
                    alt={group.Name}
