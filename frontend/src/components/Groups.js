...
const Groups = () => {
    ...
    const handleCreateGroup = async (e) => {
        ...
        try {
            ...
            await axios.post('http://localhost:8080/session', newGroupData, {
    ...
    try {
        ...
        // Map backend sessions to frontend-friendly "group" objects
        const mappedGroups = res.data.map(s => ({
            _id: s._id,
            Name: s.Name,
            Description: s.Description,
            coverImage: '', // optional, you can set default image here
            isPublic: true, // assume public for all sessions
            memberCount: s.attendance?.length || 0,
            resourceCount: s.resources?.length || 0
        }));

        // Apply tab filtering if needed (my_groups / public)
        let filteredGroups = mappedGroups;
        if (activeTab === 'my_groups') {
            // If you want, filter by sessions attended by current user
            filteredGroups = mappedGroups.filter(g => g.memberCount > 0);
        }
                        ...
                        .map((group) => (
                            ...
                            >
                                <div className="card-image-wrapper">
                                    ...
                                    <img src={group.coverImage || "https://img.freepik.com/free-vector/flat-back-school-pattern-collection_23-2149040892.jpg?semt=ais_hybrid"} alt={group.name} className="card-image" />
