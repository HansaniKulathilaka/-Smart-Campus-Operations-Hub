...
const navigate = useNavigate();
    ...
    const handleSubmit = async(e) =>{
        e.preventDefault();
        try{
            const response = await axios.post("http://localhost:8080/user2/login", {
      ...
      localStorage.setItem("role", inputs.Role );

       if (inputs.Role === "Event organizer") {
                localStorage.setItem("organizerId", response.data.userId);
                navigate("/SessionDashboard");
            } else if (inputs.Role === "SLIIT Student") {
      ...
      /* if (inputs.Role === "Event organizer") {
        localStorage.setItem("organizerId", response.data.userId);
                ...
                alert("Login Success");
                navigate(inputs.Role === "Event organizer" ? "/SessionDashboard" : "/StudentDashboard");
                //navigate("/DashboardU")
            }else{
                    alert("Login error")
                }
